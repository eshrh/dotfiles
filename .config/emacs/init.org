...
#+END_SRC
*** APL
GNU APL configuration. The main thing i do is to remap the symbol keys
to be the same as on qwerty.
#+BEGIN_SRC emacs-lisp
(sup 'gnu-apl-mode)
(require 's)
...
(load-file
 (concat user-emacs-directory "straight/build/gnu-apl-mode/gnu-apl-symbols.el"))
...
(setq gnu-apl--symbols
      (-map (lambda (sym)
              (list (-first-item sym)
                    (-second-item sym)
                    (let ((ti (-third-item sym)))
                      (alist-get ti dvlist
                                 (if-let
                                     ((item ti)
                                      (val (alist-get (downcase ti) dvlist
                                                      nil nil #'string=)))
                                     (upcase val)
                                   ti)
                                 nil #'string=))))
